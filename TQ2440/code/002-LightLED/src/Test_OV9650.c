...
static  void set_gpio_ctrl(unsigned int gpio)
{
...
void  OV9650_sccb_write_bit(unsigned char bit)
{
	if (bit)
		MAKE_HIGH(SIO_D);
	else
...
void  OV9650_sccb_writechar(unsigned char data)
{
	int i = 0;
...
void  OV9650_sccb_readchar(unsigned char *val)
{
	int i;
	int tmp = 0;
...
void  yuv_convert_rgb16(unsigned char *rawY, unsigned char *rawU,
...
void  s3c2440_camif_deinit(void)
{
	CLKCON &= ~CLKCON_CAMIF;
	CAM_CTRL = 0;
	mdelay(5);
